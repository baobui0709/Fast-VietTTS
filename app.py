...
DEFAULT_DEVICE_LABEL = "CUDA (GPU)" if torch.cuda.is_available() else "CPU"


def resolve_device(device_label: str) -> str:
    """Map UI device label to torch device."""
    if device_label == "CUDA (GPU)" and torch.cuda.is_available():
        return "cuda"
    return "cpu"


def get_model(device_label: str):
    """Lazy-load FastVietTTS per selected device."""
    global MODEL, CURRENT_DEVICE

    device = resolve_device(device_label)

    if MODEL is None or CURRENT_DEVICE != device:
        print("=" * 50)
        print(f"🚀 Loading FastVietTTS on {device}...")
        print("=" * 50)
...
def generate_speech(
    ...
    try:
        ...
        speed_badge = (
            "⚠️ Chậm hơn realtime"
            if rtf > 1.0
            else "✅ Nhanh hơn realtime"
        )

        status = (
            f"✅ Generated! | {duration:.2f}s | {language.upper()}<br>"
            f"⚡ Tốc độ: {rtf:.2f}x realtime<br>"
            f"{speed_badge}"
        )

        return (sr, audio_np), status

    except torch.cuda.OutOfMemoryError:
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
        return None, "❌ CUDA OOM: hãy giảm độ dài text hoặc dùng CPU."
...
with gr.Blocks(
    ...
    gr.HTML("""
        <div style="text-align: center; margin-bottom: 0.5rem;">
            <h1 style="margin: 0; font-size: 2rem;">🎙️ Fast-VietTTS</h1>
    ...
    with gr.Row(equal_height=True, elem_id="main-row"):
        ...
        with gr.Column(scale=1, elem_classes=["card"]):
            ...
            device_selector = gr.Radio(
                choices=["CUDA (GPU)", "CPU"] if torch.cuda.is_available() else ["CPU"],
                value=DEFAULT_DEVICE_LABEL,
                label="Device",
                info="CUDA cần GPU T4/A100. CPU chỉ dùng test ngắn."
            )
    ...
    with gr.Column(elem_classes=["output-card"]):
        ...
        with gr.Row():
            ...
            status_text = gr.HTML(label="Status")
    ...
    generate_btn.click(
        fn=generate_speech,
        inputs=[text_input, language, device_selector, ref_audio, exaggeration, cfg_weight, temperature, sentence_pause],
